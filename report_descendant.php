		...
		for($i=0; $i < count($genarray);$i++) {
			...
			if($genarray[$i]["chd"]==1) {   // the first child in this fam
				if($genarray[$i]["gen"]==0) {  // this is base person - put in left most position
					...
					//Записываем координату X в поле fst родителя
					$genarray[$par]["fst"]=$genarray[$i]["x"];       // x of first child in fam
				}

			}
			else {	//Если челоек не первый ребёнок в семье, то просто добавляем к координате предыдущего человека постоянную.
				$genarray[$i]["x"] = $genarray[$i-1]["x"] + ($hsize+$inbetween);
			}

			$z=$i;
			//Если последний ребёнок в семье
...
function move($i) {
	...
	if($direction==0) { // if vertical
		...
		$par=$genarray[$i]["par"];
		$tempx= $genarray[$i]["x"];
		$genarray[$i]["x"] = ($genarray[$i]["fst"] + $genarray[$i]["lst"])/2; //Устанавливаем координату родителя как середину между первым и последним ребёнком.

		if($genarray[$i]["gen"]!=0) {
			$q=$i;
			if($genarray[$q]["chd"] == 1) {	//Если человек - первый ребёнок в семье, записываем его координату в поле fst его родителя.
				$genarray[$par]["fst"]=$genarray[$q]["x"];
			}
			if($genarray[$q]["chd"]==$genarray[$par]["nrc"]) { //Если человек - последний ребёнок в семье.
				while($genarray[$q]["2nd"]==1) { //Находим последнего (кровного) человека и записываем его координату в поле lst родителя.
					$q--;
				}
		...
		while($genarray[$n]["gen"] == $genarray[$n-1]["gen"]) {	//Обзор семьи
			...
			else { //Иначе, добавляем предыдующую разницу
